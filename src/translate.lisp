...
;; The current is-pointer-table, each level has its own is-pointer-table,
;; a is-pointer-table is a map from a frame:access to a boolean
;; indicates whether the frame:access storing a pointer,
...
(defvar *is-pointer-table* nil)

(defun new-is-pointer-table ()
  (make-hash-table :test #'eq))

(defun mark-frame-access-as-pointer (access)
  (setf (gethash access *is-pointer-table*) t))

(defun is-frame-access-pointer (access)
  (gethash access *is-pointer-table*))
     ...
     (trivia:let-match1 (list init-ty init-tagged-ir)
       ...
       (let ((var-access (alloc-local level (ast:escape-ref-value escape-ref) target)))
         (when (is-ty-pointer init-ty)
           (mark-frame-access-as-pointer (access-frame-access var-access)))
         (list ty-env
               (insert-ir-entry
                ir-env name
                (ir-entry-var
       ...
       (mapc (lambda (decl-function)
               (let ((ir-entry (get-ir-entry new-ir-env (ast:decl-function-name decl-function))))
                 (trivia:let-match1 (ir-entry-fun formal-tys _ _ level) ir-entry
                   (let ((*is-pointer-table* (new-is-pointer-table)))
                          ...
                          (loop with acc-ir-env = new-ir-env
                                ...
                                do (setf acc-ir-env
                                   ...
                                   (when (is-ty-pointer formal-ty)
                                     (mark-frame-access-as-pointer (access-frame-access formal-access)))
...
(defun alloc-fun (body frame is-pointer-table)
  (setf *frags* (cons (frame:frag-fun body frame is-pointer-table) *frags*)))
...
(defun translate-program (prog target)
  ...
  (let ((*frags* nil)
        ...
        (*is-pointer-table* (new-is-pointer-table))
